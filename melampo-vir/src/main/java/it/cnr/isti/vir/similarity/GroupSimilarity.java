...
public abstract class GroupSimilarity<F> implements Similarity<F> {
	...
	public GroupSimilarity(Properties properties) throws SimilarityOptionException {
		this(properties.getProperty("simOption"));
	}
	
	public GroupSimilarity(String opt) throws SimilarityOptionException {
		if ( opt != null ) {
			if ( opt.equals("def")) {
				option = optFt1;
